...
def test_basic_parsing(yeti, resources_dir):
    ...
    config_manager.parse_config_file(join(resources_dir, "config_manager", "parsing.conf"))
...
def test_module_loading(yeti, context, resources_dir):
    config_manager = yeti.ConfigManager()
    config_manager.parse_config_file(join(resources_dir, "config_manager", "basic.conf"))
